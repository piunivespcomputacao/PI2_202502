...
def analisar_compressor(request):
    ...
    if request.method == 'POST':
        compressor_id = request.POST.get('compressor_id')
        
        try:
            # Busca o objeto Compressor no banco de dados
            compressor = Compressor.objects.get(id=compressor_id)
            
            problemas_encontrados = []
            
            # --- Lógica de regras (A sua "IA") ---  #EDITAR A RESPOSTA DOS PROBLEMAS ENCONTRADOS
            # 1. Análise de RPM
            ...
            # 7. Análise de Vibração (gaccx, gaccy, gaccz)
            if compressor.gaccx > VALORES_REFERENCIA['gaccx_normal_max'] or \
               ...
               compressor.gaccz > VALORES_REFERENCIA['gaccz_normal_max']:
                problemas_encontrados.append("Vibração do motor acima do padrão. Verifique o balanceamento e alinhamento.")

            # 8. Análise de Vibração (haccx, haccy, haccz)
            if compressor.haccx > VALORES_REFERENCIA['haccx_normal_max'] or \
               ...
               compressor.haccz > VALORES_REFERENCIA['haccz_normal_max']:
                problemas_encontrados.append("Vibração do cilindro acima do padrão. Possível desgaste interno.")

            # 9. Análise da Válvula de Escape e Motor
            if compressor.exvalve == "dirty":
                problemas_encontrados.append("Válvula de escape suja. Afeta a eficiência do ciclo.")
            if compressor.acmotor == "unstable":
                problemas_encontrados.append("Motor AC instável. Pode indicar problemas elétricos.")
            ...
            resultado_analise = {
                ...
                'detalhes_parametros': {
                    ...
                    'Ruído': compressor.noise_db,
                    'Temperatura de Saída': compressor.outlet_temp,
                    'Vibração (Motor)': f"X:{compressor.gaccx}, Y:{compressor.gaccy}, Z:{compressor.gaccz}",
                    'Vibração (Cilindro)': f"X:{compressor.haccx}, Y:{compressor.haccy}, Z:{compressor.haccz}",
                    'Válvula de Escape': compressor.exvalve,
                    'Motor AC': compressor.acmotor
                }
            }
    ...
    contexto = {'resultado': resultado_analise}
    return render(request, 'qualidade/analise.html', contexto)